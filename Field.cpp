...
static WorldState& world = SWorldState::getInstance();
 ...
 * */
void Field::draw_visible (const Matrix4D& headToFieldT) const{
    ...
    for(const Line6f& l : list_segments) {
        ...
        roboviz->drawLine(s.x, -s.y, s.z, e.x, -e.y, e.z, 1, 0.8,0,0, NULL);
    }  
    ...
    for(const auto& s : list_known_segments){
		...
		roboviz->drawAnnotation(&line_name,mid.x,-mid.y,mid.z, 0,1,0,NULL);
        roboviz->drawLine(s.point[0].absPos.x, -s.point[0].absPos.y, s.point[0].absPos.z, 
                          s.point[1].absPos.x, -s.point[1].absPos.y, s.point[1].absPos.z, 3, 0,0.8,0, NULL);
	}

    for(const auto& m : list_known_markers){
        string line_name(m.fieldPt->name);
		roboviz->drawAnnotation(&line_name,m.absPos.x, -m.absPos.y, m.absPos.z+1, 1,0,0,NULL);
        roboviz->drawLine(m.absPos.x, -m.absPos.y, m.absPos.z, 
                          m.absPos.x, -m.absPos.y, m.absPos.z+0.5, 1, 0.8,0.8,0.8, NULL);
	}
    ...
    Vector3f b = headToFieldT * agent::worldState->ball->relPosition;


    roboviz->drawLine(me.x, -me.y, me.z, me.x,     -me.y, me.z+0.5, 2,1,0,0,NULL);
    roboviz->drawLine(me.x, -me.y, me.z, me.x-0.2, -me.y, me.z+0.2, 2,1,0,0,NULL);
    roboviz->drawLine(me.x, -me.y, me.z, me.x+0.2, -me.y, me.z+0.2, 2,1,0,0,NULL);

    roboviz->drawLine(b.x, -b.y, b.z, b.x,     -b.y, b.z+0.5, 2,1,1,0,NULL);
    roboviz->drawLine(b.x, -b.y, b.z, b.x-0.2, -b.y, b.z+0.2, 2,1,1,0,NULL);
    roboviz->drawLine(b.x, -b.y, b.z, b.x+0.2, -b.y, b.z+0.2, 2,1,1,0,NULL);
...
void Field::gather_ground_markers(){
    ...
    const Types::Side feetSide[2] = {Types::iLeft, Types::iRight};
	const Types::BodyParts feetPart[2] = {Types::ilLFoot, Types::ilRFoot};

	for(int i=0; i<2; i++){
		if( agent::model->getFootTouch(feetSide[i]) ){ //if this foot is touching the ground

			Vector3f contactAux = agent::model->getFootContact(feetSide[i]); //contact point using strange coordinate system
			Vector3f contactpt = Vector3f(contactAux.y,-contactAux.x,contactAux.z); // fix coordinate system for both feet

            Vector3f relPos = agent::model->getRelPositionOfBodyPoint(agent::model->getBodyPart(feetPart[i]),contactpt);
...
void Field::update(){
    ...
    for(const auto l : world.landmarks){
        ...
        if     (l->position.x == -15 && l->position.y == -10) {l8 = &list_8_landmarks::_corner_mm; fp = &cFieldPoints::corner_mm;}
        else if(l->position.x == -15 && l->position.y == +10) {l8 = &list_8_landmarks::_corner_mp; fp = &cFieldPoints::corner_mp;}
        else if(l->position.x == +15 && l->position.y == -10) {l8 = &list_8_landmarks::_corner_pm; fp = &cFieldPoints::corner_pm;}
        else if(l->position.x == +15 && l->position.y == +10) {l8 = &list_8_landmarks::_corner_pp; fp = &cFieldPoints::corner_pp;}
        else if(l->position.x == -15 && l->position.y < 0)    {l8 = &list_8_landmarks::_goal_mm;   fp = &cFieldPoints::goal_mm;  }
        else if(l->position.x == -15 && l->position.y > 0)    {l8 = &list_8_landmarks::_goal_mp;   fp = &cFieldPoints::goal_mp;  }
        else if(l->position.x == +15 && l->position.y < 0)    {l8 = &list_8_landmarks::_goal_pm;   fp = &cFieldPoints::goal_pm;  }
        else if(l->position.x == +15 && l->position.y > 0)    {l8 = &list_8_landmarks::_goal_pp;   fp = &cFieldPoints::goal_pp;  }
        else{ cout << "Unexpected landmark position: " << l->position << endl; return; } 

        if(l->seen){   
            l8->set_relPos(l->relPosition);
            l8->visible = true;

            sMarker seen_mark(fp, l->relPosition);
            list_landmarks.push_back(seen_mark);
            list_known_markers.push_back(seen_mark);

            if     (l->name[0]=='G'){ list_landmarks_goalposts.push_back(seen_mark); }
            else if(l->name[0]=='F'){ list_landmarks_corners.push_back(  seen_mark); }
            else{ cout << "Unexpected landmark name: " << l->name << endl; return; } 
        }else{
            l8->visible = false;
        }
    }
    ...
    for(const WorldState::lineVector3f& l : world.lineVector) {
        //Protection against a rare server bug where lines with nans are sent
        if(isnan(l.start.x) || isnan(l.start.y) || isnan(l.start.z) || isnan(l.end.x) || isnan(l.end.y) || isnan(l.end.z)){ continue; }
