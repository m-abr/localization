...
void LocalizerV2::run(){
	...
	//Ball position stats
	if(agent::worldState->ball->seen){
		counter_ball += stats_sample_position_error(prelimHeadToField * agent::worldState->ball->relPosition,
			agent::worldState->ballPosFromMessage, errorSum_ball);
	}
 ...
 * @param Yvec  output y-axis orientation vector (original coordinate system)
 */
...
/**
 * WORKFLOW: 1
 * See workflow descrition on header
 */ 
...
double LocalizerV2::map_error_logprob(const gsl_vector *v, void *params){
	...
	Matrix4D& transfMat = agent::localizer->prelimHeadToField;
...
double LocalizerV2::map_error_2d(const gsl_vector *v, void *params){
	...
	for(const Line6f& l : fd.list_segments){
		...
		Vector3f ls_legacy(ls.x, -ls.y, ls.z);
		Vector3f le = transfMat * l.endc; 
		Vector3f le_legacy(le.x, -le.y, le.z);
	...
	for(const Field::sMarker& m : fd.list_landmarks){
		...
		Vector3f lpt_legacy(lpt.x, -lpt.y, lpt.z);
...
bool LocalizerV2::fine_tune(float initial_angle, float initial_x, float initial_y){
	...
	counter_fineTune += stats_sample_position_error(Vector3f(initial_x,initial_y,prelimHeadToField.get(11)), agent::worldState->myPosFromMessage, errorSum_fineTune_before);
	...
	stats_sample_position_error(Vector3f(initial_x,initial_y,prelimHeadToField.get(11)), agent::worldState->myPosFromMessage, errorSum_fineTune_euclidianDist);
	...
	stats_sample_position_error(prelimHeadToField.toVector3f(), agent::worldState->myPosFromMessage, errorSum_fineTune_probabilistic);
...
bool LocalizerV2::guess_xy(){
	...
	Vector3f Zvec(prelimHeadToField.get(2,0), prelimHeadToField.get(2,1), prelimHeadToField.get(2,2));
	Vector last_known_position(head_position.x, -head_position.y); //convert from legacy to original
 ...
 * And translate data to legacy coordinate system
 */
void LocalizerV2::commit_everything(){

	final_headTofieldTransform = prelimHeadToField; //Full transformation (relative to absolute)

	//Translate to legacy coordinate system (erase this cycle to get everything according to the original coordinate system)
	for(int i=4; i<8; i++){ //Invert 2nd row
		final_headTofieldTransform.set(i,-final_headTofieldTransform.get(i));
	}
...
void LocalizerV2::print_report() const{
	...
	printf("- Total:               %i \n", st[RUNNING]);
	printf("- Successful:          %i \n", st[DONE]);
	printf("- Blind agent:         %i (optimal value: 2/3 of total)\n", st[BLIND]);
 ...
 * @param position estimated agent's position (original coordinate system)
 * @param cheat actual position provided by server (legacy coordinate system)
...
int LocalizerV2::stats_sample_position_error(const Vector3f sample, const Vector3f& cheat, double error_placeholder[]){
	...
	if(agent::worldState->myPosFromMessage == Vector3f(0,0,0)) return 0;

	double x_err = sample.x - cheat.x;
	double y_err = sample.y + cheat.y; //(according to original coord sys)
